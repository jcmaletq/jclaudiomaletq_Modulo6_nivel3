...
/*	
	...
	*/
	...
	public void createVolunteer(String name, String address, String phone, String description, double gob_pay) throws Exception{
		Volunteer volunteer = new Volunteer(name, address, phone,  description, gob_pay);
		double max;
		max = maxmin.getmaxmin("Volunteer_max");
		if (volunteer.salaryValidation(gob_pay,max)) {
			repository.addMember(volunteer);
		}
		else {
			throw new errorayuda(name,gob_pay,max);
		}	
	...
	public void payBonus_All_Emplloyees() {
		List<AbsStaffMember> miembros = this.repository.getAllMembers();
		for(AbsStaffMember m: miembros)
		{
			m.bonus();
...
class errorsalary extends Exception { 
	public errorsalary (String name, double min, double max) { 
		super();
		System.out.println("El salario anual de "+name+ " debe ser superior a "+min+" e inferior a "+max);
		}
}
class errorayuda extends Exception { 
	public errorayuda(String name,double gob_pay, double max) {  
		super();
		System.out.println("La ayuda de "+name+ " es de "+gob_pay+" debe ser inferior a "+max);
		}
}
